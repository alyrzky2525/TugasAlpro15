...
public:
    bool login()
    {
        ...
        if (validateCredentials(u, p, role) && role == "admin")
        {
            ...
            password = p;
            currentUsername = u;
            cout << "LOGIN BERHASIL -> [" << currentUsername << "]\n";
            return true;
        }
        return false;
    }
    ...
    void editProduk()
    {
        ...
        while (getline(fin, line))
        {
            ...
            if (c == kode)
            {
                ...
                if (pilihBrand == "1")
                    brand = "Wardah";
                else if (pilihBrand == "2")
                    brand = "Emina";
                else if (pilihBrand == "3")
                    brand = "Skintific";
                else if (pilihBrand == "4")
                    brand = "Somethinc";
                else if (pilihBrand == "5")
                    brand = "Garnier";
                else if (pilihBrand == "6")
                    brand = "Azarine";
                else if (pilihBrand == "7")
                    brand = "Maybelline";
                else if (pilihBrand == "8")
                    brand = "Implora";
                else if (pilihBrand == "9")
                    brand = "Pinkflash";
                else if (pilihBrand == "10")
                    brand = "Scarlett";
                else if (pilihBrand == "11")
                    brand = "Hada Labo";
                else if (pilihBrand == "12")
                    brand = "Nivea";
                else if (pilihBrand == "13")
                    brand = "Vaseline";
                else if (pilihBrand == "14")
                    brand = "HMNS";
...
public:
    void tambahPromo()
    {
        ...
        if (promoCodeExists(code))
        {
            cout << "Kode promo sudah ada.\n";
            return;
        }
        ...
        string m = inputLine("Hanya untuk member? (y/n): ");
        string memberOnly = (m == "y" || m == "Y") ? "1" : "0";
        ofstream fout(PROMOS_FILE.c_str(), ios::app);
        if (!fout)
        {
            cout << "Gagal menulis file promo.\n";
            return;
        }
    ...
    bool cekPromo()
    {
        ...
        if (valid)
            cout << "Promo " << outCode << " berlaku: " << pct << "%\n";
        else
            cout << "Promo tidak valid atau tidak dapat digunakan.\n";
        return valid;
    }
    ...
    bool promoCodeExists(const string &code)
    {
        ifstream fin(PROMOS_FILE.c_str());
        if (!fin)
            return false;
        string line;
        while (getline(fin, line))
    ...
    void tampilPromo()
    {
        ...
        while (getline(fin, line))
        {
            ...
            string pct = line.substr(p2 + 1, p3 - p2 - 1);
    ...
    {
        Promo promo;
        string code = inputLine("Kode promo yang diedit: ");
        if (!promo.promoCodeExists(code))
        {
            cout << "Kode tidak ditemukan.\n";
            return;
        }
        ...
        if (!fin)
        {
            cout << "File promo tidak ditemukan.\n";
            return;
        }
        string all;
        string line;
        while (getline(fin, line))
        {
            ...
            if (c == code)
            {
                ...
                string desc = inputLine("Deskripsi baru: ");
                string pcts = inputLine("Persentase baru: ");
                string start = inputLine("Mulai baru (YYYY-MM-DD): ");
                string end = inputLine("Selesai baru (YYYY-MM-DD): ");
                string m = inputLine("Hanya untuk member? (y/n): ");
                string memberOnly = (m == "y" || m == "Y") ? "1" : "0";
        ...
        fin.close();
        ofstream fout(PROMOS_FILE.c_str(), ios::trunc);
        if (!fout)
        {
            cout << "Gagal menulis file promo.\n";
            return;
        ...
        fout << all;
        cout << "Promo diperbarui.\n";
    }

    void hapusPromo()
    {
        Promo promo;
        string code = inputLine("Kode promo yang dihapus: ");
        if (!promo.promoCodeExists(code))
        {
            cout << "Kode tidak ditemukan.\n";
            return;
        }
        if (!confirmAction(string("Hapus promo '") + code + "' ?"))
            return;
        ifstream fin(PROMOS_FILE.c_str());
        string all;
        string line;
    ...
    double applyPromo(const string &promoCode, const string &date, bool isMember, bool &valid, string &outPromoCode)
    {
        ...
        while (getline(fin, line))
        {
            ...
            string code = line.substr(0, p1);
            string desc = line.substr(p1 + 1, p2 - p1 - 1);
            string pcts = line.substr(p2 + 1, p3 - p2 - 1);
            string start = line.substr(p3 + 1, p4 - p3 - 1);
            string end = line.substr(p4 + 1, p5 - p4 - 1);
            string memberOnly = line.substr(p5 + 1);
            if (code != promoCode)
    ...
    void cetakKeFile(int id)
    {
        ...
        while (getline(fin, line))
        {
            ...
            while (pos < items.size())
            {
                ...
                size_t a = chunk.find(':');
                size_t b = chunk.find(':', a + 1);

                string code = chunk.substr(0, a);
                string qty = chunk.substr(a + 1, b - a - 1);
                string price = chunk.substr(b + 1);
...
public:
    void tambahShift()
    {
        ...
        string s = inputLine("Pilih tipe shift (1/2/3): ");
        string shiftName;
        if (s == "1")
            shiftName = "Opening";
        else if (s == "2")
        ...
        string emp = inputLine("Username pegawai: ");
        if (emp.empty())
        {
            cout << "Username kosong.\n";
            return;
        ...
        fin.close();
        ofstream fout(SHIFTS_FILE.c_str(), ios::app);
        if (!fout)
        {
            cout << "Gagal menulis file shift.\n";
            return;
        }
...
public:
    ...
    void tampilPresensi()
    {
        ...
        if (user.empty())
            ...
            listAttendanceForEmployee(user);
    }
};
...
int main()
{
    ...
    while (true)
    {
        ...
        else if (choice == "2")
        {
            ...
            cout << " - Role: " << role << "\n";
            if (!confirmAction("Konfirmasi registrasi?"))
            {
            ...
            if (registerUser(username, password, role))
                ...
                cout << "Gagal registrasi (tidak dapat menulis file).\n";
        }
        else if (choice == "3")
        {
            ...
            if (!confirmAction("Lanjut ganti password?"))
            {
                cout << "Perubahan password dibatalkan.\n";
                continue;
            }
            if (changePassword(username, np))
                cout << "Password berhasil diubah.\n";
...
void ensureDefaultData()
{
    ...
    ifstream rfin(PROMOS_FILE.c_str());
    if (rfin)
    {
        string any;
        if (getline(rfin, any))
    ...
    ifstream mfin(MEMBERS_FILE.c_str());
    if (mfin)
    {
        string any;
        if (getline(mfin, any))
        ...
        else if (c == "5")
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (getline(fin, line))
                    {
                        ...
                        if (kode == code)
                        {
                            ...
                            else
                            {
                                ...
                                // notify low stock
                                if (s == 0)
                                    cout << "Transaksi berhasil. Stok habis untuk produk " << kode << "\n";
                                else if (s <= 5)
                                    cout << "Transaksi berhasil. Peringatan: stok menipis untuk " << kode << ": " << s << "\n";
                                else
                                    cout << "Transaksi berhasil. Sisa stok: " << s << "\n";
                                // rebuild line
                    ...
                    ofstream fout(PRODUCTS_FILE.c_str(), ios::trunc);
                    if (!fout)
                        cout << "Gagal menyimpan perubahan stok.\n";
                    else
                        fout << all;
                }
            }
...
void kelolaKategori()
{
    ...
    while (true)
    {
        ...
        getline(cin, c);
        if (c == "1")
            produk.tambahkankategori();
        else if (c == "2")
            produk.daftarKategori();
        else if (c == "3")
            break;
        else
            cout << "Pilihan tidak dikenali.\n";
...
{
    if (!isValidDateFormat(expDate) || !isValidDateFormat(curDate))
        return false;
...
bool checkoutCart(const string &curDate)
{
    ...
    for (int ci = 0; ci < cartCount; ++ci)
    {
        ...
        if (foundPos == string::npos)
        {
            cout << "Produk " << code << " tidak ditemukan pada file.\n";
            return false;
        }
        size_t p1 = foundPos;
        ...
        int stokVal = 0;
        for (size_t i = 0; i < stokStr.size(); ++i)
            if (stokStr[i] >= '0' && stokStr[i] <= '9')
                stokVal = stokVal * 10 + (stokStr[i] - '0');
        if (!curDate.empty() && isExpired(expired, curDate))
        {
            cout << "Produk " << code << " sudah expired/tidak layak jual.\n";
            return false;
        }
        if (stokVal < needQty)
        {
            cout << "Stok tidak cukup untuk produk " << code << ". (butuh " << needQty << ", tersedia " << stokVal << ")\n";
            return false;
        }
    ...
    if (!member.empty())
    {
        ...
        {
            cout << "Member tidak ditemukan, akan diproses sebagai non-member.\n";
            member.clear();
        }
    }
    ...
    if (!promoIn.empty())
    {
        ...
        if (!promoValid)
        {
            cout << "Promo tidak valid atau tidak dapat digunakan.\n";
            promoIn.clear();
            appliedPromoCode.clear();
            promoPct = 0.0;
        }
        else
            cout << "Promo " << appliedPromoCode << " diterapkan: " << promoPct << "%\n";
    }
...
void analisisPenjualanPerKategori()
{
    ...
    for (int i = 0; i < catRevenueSize; i++)
    {
        ...
        if (totalPendapatan != 0)
            persen =
                (catRevenue[i].value * 100.0) / totalPendapatan;
